...
#if defined(NDEBUG)
# error "polishcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x68a5b7d21fefef20d108b4ceef8c05fa67f72ba1b64395c57961e48022af7d06");
...
const string strMessageMagic = "polishcoin Signed Message:\n";
...
{
    // polishcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 750000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 60 * 60; // PolishCoin: every 4 hours
static const int64 nTargetSpacing = 60; // PolishCoin: 1 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // polishcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x68a5b7d21fefef20d108b4ceef8c05fa67f72ba1b64395c57961e48022af7d06");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1391982922;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3042786;
...
unsigned char pchMessageStart[4] = { 0xfc, 0xd9, 0xb7, 0xdd }; // polishcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("polishcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("polishcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static polishcoinMiner(CWallet *pwallet)
{
    printf("polishcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("polishcoin-miner");
    ...
    try { loop {
        ...
        printf("Running polishcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("polishcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&polishcoinMiner, pwallet));
}
